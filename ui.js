...
export function showMeteorStormResult(data) {
  ...
  if (legendaryEl) legendaryEl.textContent = data.legendaryGeodes;
  if (rareEl) rareEl.textContent = data.rareGeodes;
  if (commonEl) commonEl.textContent = data.commonGeodes;
  if (xpEl) xpEl.textContent = data.totalXP;
...
function showAdminPanel() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('adminTriggerSmelt')?.addEventListener('click', () => {
    ...
    document.getElementById('adminTriggerStorm')?.addEventListener('click', () => {
...
export function renderEventsTab() {
  ...
  if (activeEvent && phase === 'active') {
    if (activeEvent.type === 'great_smelt') {
      ...
      html += `
        <div class="card" style="border: 2px solid rgba(180,0,255,0.4); background: rgba(100,0,200,0.05); position: relative; overflow: hidden;">
          ...
          <div class="event-icon" style="font-size:72px; margin-bottom:16px;">${activeEvent.icon}</div>
          <div class="event-title" style="color: #B400FF; font-size: 22px; margin-bottom: 8px;">${activeEvent.name}</div>
          <div class="event-desc" style="color: var(--text-primary); font-size: 14px; line-height: 1.6; margin-bottom: 16px;">${activeEvent.longDescription || activeEvent.description}</div>
          ...
          <button class="btn" id="enterMeteorStormBtn" style="width: 100%; background: linear-gradient(135deg, #B400FF, #FFD700); box-shadow: 0 4px 25px rgba(180,0,255,0.4);">
            ☄️ ВОЙТИ В ШТОРМ
          </button>
          
          <div style="margin-top: 12px; text-align: center; color: var(--text-muted); font-size: 11px;">
            Лови метеориты: ✨ Легендарные · 🔥 Редкие · ☄️ Обычные<br>
            Обменяй на жеоды: 2 легендарных / 4 редких / 6 обычных
          </div>
        </div>
      `;
    }
    ...
    html += `
      <div class="event-placeholder">
        ...
        <div class="event-title">Ивент завершён</div>
        <div class="event-desc">Дождитесь следующего ивента.</div>
    ...
    html += `
      <div class="event-placeholder">
        ...
        <div class="event-desc">Ивенты запускаются автоматически. Проверяйте вкладку Ивентов!</div>
      </div>
    `;
  }
  ...
  if (enterMeteorStormBtn) {
    enterMeteorStormBtn.addEventListener('click', () => {
      openMeteorStorm();
    });
  }
